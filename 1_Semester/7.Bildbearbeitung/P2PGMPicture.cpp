...
void P2PGMPicture::loadData() {
    ...
    for (int row = 0; row < this->height; row++) {
        ...
        this->data[row] = new Pixel[this->width];
        for (int col = 0; col < this->width; col++) {
            int tempValue = inputHandle->readInt();
            // Pruefe den Wert, ob dieser im Wertebereich liegt
            if(tempValue > this->greydepth)
            {
                stringstream ssErrortext;
                ssErrortext << "Current greyvalue(" << tempValue <<
                        ") higher than max greydepth (" << this->greydepth << ")";
                string errorText = ssErrortext.str();
                throw InvalidPictureException(errorText);
            }
            this->data[row][col] = tempValue;
        }
    }
}
